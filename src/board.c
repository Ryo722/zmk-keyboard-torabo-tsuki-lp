...
// SLEEP3 は最終段階で本来 8x だが、Mac host との BLE スケジューラ競合回避のため 4x に抑制
// （split interval が 60ms 付近になると nRF52 controller で Mac 側 30-50ms 接続と
//  時間スロット衝突し、host 側パケット欠落→supervision timeout→切断に至るケースがあるため）
#define SLEEP3_CONN_INTERVAL (CONFIG_ZMK_SPLIT_BLE_PREF_INT*4)
...
// SLEEP3 latency も SLEEP2 相当に合わせる（interval を上げないならより小さくする意味も薄い）
#define SLEEP3_CONN_LATENCY ((CONFIG_ZMK_SPLIT_BLE_PREF_LATENCY+3)/4)
